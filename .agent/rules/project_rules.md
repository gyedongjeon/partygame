...
## 2. Mandatory Testing Strategy (TDD Approach)
- **Test-Driven Development (TDD) Priority**: No production code shall be written before a corresponding failing test exists. Adhere strictly to the **Red-Green-Refactor** cycle:
    ...
    - **Green**: Write the simplest possible code to make the test pass (prioritize functionality over perfection at this stage).
...
- **Behavior-Centric Testing**: Focus on testing the behavior and public API rather than internal implementation details to ensure tests are resilient to refactoring.
- **Edge Cases & Failure Modes**: Tests must cover happy paths, boundary conditions, and potential failure modes (e.g., null inputs, exceptions).
- **Test Readability**: Tests are first-class citizens. They must be as readable, descriptive, and maintainable as the production code itself.
