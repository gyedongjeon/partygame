...
export class LobbyService {
    ...
    updateSettings(roomId: string, hostId: string, settings: Partial<GameSettings>): Room {
        const room = this.rooms.get(roomId);
        if (!room) throw new NotFoundException('Room not found');
        ...
        if (newSettings.maxPlayers < 2) throw new Error('Max players must be at least 2');
        if (newSettings.imposterCount >= newSettings.maxPlayers)
            throw new Error('Imposter count must be less than max players');
        if (newSettings.timeLimitDuration < 10) throw new Error('Time limit must be at least 10s');
